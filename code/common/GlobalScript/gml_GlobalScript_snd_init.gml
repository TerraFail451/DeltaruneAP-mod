...
function snd_init(arg0)
{
    ...
    var _embedded = [snd_usefountain, cyber_battle_backing_solo, mus_undynescary, snd_flowery_bromide_r, snd_flowery_bromide_f, snd_pink_stretch_2_troubled, snd_pink_stretch_2_fixed];
    var _string = "";
    var _is_embedded = 0;

    if (scr_array_contains(_embedded, arg0))
    {
        _string = "\"" + arg0 + "\"";
        _is_embedded = 1;
    }
    ...
    {
        if (global.AP_unrandomized_current_song != _string)
        {
            if ((!scr_array_contains(_ignored, _string)) && (scr_array_contains(global.AP_randomized_music_list, _string)))
                var _song = AP_get_random_song(_string);
            else
                var _song = arg0;
            
            global.AP_unrandomized_current_song = _string;
            global.AP_randomized_current_song = _song;
        }
        else
        {
            var _song = global.AP_randomized_current_song;
        }
    }
    else
    {
