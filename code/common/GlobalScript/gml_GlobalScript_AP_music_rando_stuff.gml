...
function AP_get_random_song(arg0)
{
    ...
    {
        _index = scr_array_get_index(global.AP_randomized_music_list, arg0);
        _song = global.AP_randomized_music_list_shuffled[_index];
    }

    if (global.AP_randomize_music == 3)
    {
        _index = irandom_range(1, (array_length(global.AP_randomized_music_list) - 1));
...
function AP_shuffle_music_rando_list()
{
    ...
    if (global.AP_randomize_music == 1)
    {
        _music_file = AP_get_save_folder_prefix() + "music.txt";
    }

    if (global.AP_randomize_music == 2)
    {
        if (!variable_global_exists("chapter"))
            var _chapter_string = 0;
        else
            var _chapter_string = global.chapter;
        
        _music_file = AP_get_save_folder_prefix() + "music_ch" + _chapter_string + ".txt";
    }

    if (!file_exists(_music_file))
    {
        var _shuffled_list = scr_array_shuffle(global.AP_randomized_music_list);
...
function AP_get_randomized_music_list()
{
    ...
    if (_included_chapters[0] == 1) // CHAPTER 1
    {
        ...
        if (global.AP_include_odd_music >= 1)
            array_push(_song_array, "bird.ogg", "fanfare.ogg", "GALLERY.ogg", "snd_usefountain", "w.ogg", "elevator.ogg", "charjoined.ogg", "ocean.ogg");
    }
    ...
    var _songs = scr_array_unique(_song_array);

    return _songs;
}

function scr_array_unique(array)
{
...
function scr_array_shuffle(array, offset = 0, length = array_length(array))
{
    var result = [];
...
function scr_array_get_index(array, value)
{
    for (var i = 0; i < array_length(array); i++)
    {
        if (array[i] == value)
